...
const RATE_LIMIT_BACKOFF_MAX_MS = parseInt(
  process.env["OPENAI_RATE_LIMIT_BACKOFF_MAX_MS"] || "60000",
  10,
);
          ...
          try {
            ...
            if (isRateLimit) {
              if (attempt < MAX_RETRIES) {
                ...
                // if provided.
                let delayMs = computeBackoffDelay(
                  attempt,
            ...
            if (isClientError) {
              const msgText = (() => {
                ...
                }. Error details: ${details}. Please verify your settings and try again.`;
              })();
